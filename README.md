...
3. Prefer async I/O over sync I/O. Threads that wait for a database query to complete can handle other requests in the meantime when the request is performed asynchronously. This prevents thread starvation under high load and allows your web service to scale better. If you really decide to use synchronous access to RavenDB, there are the `ReadOnlySession` and `Session` base classes that you can use to implement abstractions. You must register the store and document session on your own against the DI container in this case.
